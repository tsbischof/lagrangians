...
def apply_substitutions(S, items):
    result = S
    for item, replacement in items:
        result = re.sub(item, replacement, result)

    return(result)

if __name__ == "__main__":
    file = " ".join(open("platform_with_springs").read().split("\n"))
    file = apply_substitutions(file, [["\(t\)", ""], ["\\\\", ""], ["\[", ""],\
              ["\]", ""], ["Power", "pow"], ["==", "="], ["\t", ""], [" ", ""]])
    file = file.lower()

    a = ast.parse(file, "", "exec")
    print(a)
