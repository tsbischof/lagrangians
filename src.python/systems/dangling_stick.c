...
void dangling_stick_derivs(double *r, double *drdt) {
	drdt[DR] = (-2*r[G]*pow(r[M1],2)-3*r[G]*r[M1]*r[M2]-r[G]*pow(r[M2],2)+2*r[K]*r[M1]*r[R_0]+r[K]*r[M2]*r[R_0]+2*r[G]*pow(r[M1],2)*cos(r[PHI1])+2*r[G]*r[M1]*r[M2]*cos(r[PHI1])+r[G]*r[M1]*r[M2]*cos(2*(r[PHI1]-r[PHI2]))+r[G]*pow(r[M2],2)*cos(2*(r[PHI1]-r[PHI2]))-r[K]*r[M2]*r[R_0]*cos(2*(r[PHI1]-r[PHI2]))+r[R]*(-(r[K]*(2*r[M1]+r[M2]-r[M2]*cos(2*(r[PHI1]-r[PHI2]))))+2*r[M1]*(r[M1]+r[M2])*pow(r[DPHI1],2))+2*r[L]*r[M1]*r[M2]*cos(r[PHI1]-r[PHI2])*pow(r[DPHI2],2))/(2.*r[M1]*(r[M1]+r[M2]));
	drdt[DPHI1] = -(2*r[G]*pow(r[M1],2)*sin(r[PHI1])+2*r[G]*r[M1]*r[M2]*sin(r[PHI1])+r[G]*r[M1]*r[M2]*sin(2*(r[PHI1]-r[PHI2]))+r[G]*pow(r[M2],2)*sin(2*(r[PHI1]-r[PHI2]))-r[K]*r[M2]*r[R_0]*sin(2*(r[PHI1]-r[PHI2]))+r[K]*r[M2]*r[R]*sin(2*(r[PHI1]-r[PHI2]))+4*r[M1]*(r[M1]+r[M2])*r[DR]*r[DPHI1]+2*r[L]*r[M1]*r[M2]*sin(r[PHI1]-r[PHI2])*pow(r[DPHI2],2))/(2.*r[M1]*(r[M1]+r[M2])*r[R]);
	drdt[DPHI2] = ((r[G]*(r[M1]+r[M2])-r[K]*r[R_0]+r[K]*r[R])*sin(r[PHI1]-r[PHI2]))/(r[L]*r[M1]);
	drdt[R] = r[DR];
	drdt[PHI1] = r[DPHI1];
	drdt[PHI2] = r[DPHI2];
	drdt[R_0] = 0;
...
double dangling_stick_U(double *r) {
	return(r[G]*r[L]*r[M2]*(1-cos(r[PHI2]))+r[G]*(r[M1]+r[M2])*(1-cos(r[PHI1]))*r[R]+(r[K]*pow(-r[R_0]+r[R],2))/2.);
}
